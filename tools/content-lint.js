...
function validateFrontmatter(f, data, langFromPath) {
  ...
  if ('lang' in data && data.lang) {
    if (typeof data.lang !== 'string' || !SUPPORTED_LANGS.has(data.lang)) {
      ...
      errs.push(`"lang" (${data.lang}) does not match folder language (${langFromPath}).`);
    }
  }
...
function validateMarkdownFileBytesAndChars(filePath) {
  ...
  try {
    ...
    if (size > MD_MAX_BYTES) {
      errs.push(`Markdown file too large: ${size} bytes (max ${MD_MAX_BYTES}).`);
    }
    ...
    errs.push(`Unable to read file for size/charset validation: ${(e && e.message) || e}`);
  }
...
async function main() {
  ...
  for (const f of files) {
    ...
    if (!langFromPath || !SUPPORTED_LANGS.has(langFromPath)) {
      ghaError(f, `Invalid language folder "${langFromPath ?? '(none)'}". Must be one of: ${[...SUPPORTED_LANGS].join(', ')}`);
      totalErrors++;
      continue;
    }
